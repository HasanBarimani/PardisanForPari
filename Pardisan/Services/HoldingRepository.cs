...
namespace Pardisan
{
    public class HoldingRepository : IHoldingRepository
    {
        ...
        public async Task<Response<string>> Add(UpsertHoldingVM input)
        {
            ...
            BlogModel.Image = pathShow + FileUploader.UploadImage(input.Image, Path.Combine(host, path), Path.Combine(host, pathWaterMark), false, compression: 25).result;
        ...
        public async Task<Response<List<HoldingVM>>> GetAll()
        {
            ...
            var find = _context.Holdings.Where(d => d.IsActive == true).Select(d => new HoldingVM()
            {
                Id = d.Id,
                Title = d.Title,
                Image = d.Image,
            ...
            return new Response<List<HoldingVM>>(find.ToList());
        }
