...
namespace Pardisan.Services
{
    public class EstateRepository : IEstateRepository
    {
        ...
        {
            var Floors = JsonConvert.DeserializeObject<List<Pardisan.ViewModels.Estate.FloorVM>>(input.floors);
            ...
            foreach (var i in Floors)
            {
                ...
                _context.Floors.Add(flr);
                _context.SaveChanges();
                foreach (var z in i.units)
                {

                    var unit = new EstateUnits()
                    {

                        MergedUnitId = z.mergedUnitId,
                        FloorId = i.id,
                        IsEmpty = z.isEmptyUnit,
                        ClientIdForUnit = z.id,
                        Meterage = z.Meterage,
                        name = z.name,


                    };
                    _context.EstateUnits.Add(unit);
                    _context.SaveChanges();
        ...
        public async Task<UpsertEstateVM> BadCodeGetById(int id)
        {

            var data = await _context.Estates.Include(x => x.Images).FirstOrDefaultAsync(d => d.Id == id);

            return CreateViewModelFromBlog(data);
        ...
        private UpsertEstateVM CreateViewModelFromBlog(Estate estate)
        {
            ...
            var viewModel = new UpsertEstateVM()
            {
                ...
                floorCounts = estate.FloorCounts,
            ...
            var floors = _context.Floors.Include(x => x.EstateUnits).Where(x => x.EstateId == estate.Id).Select(d => new FloorVM()
            {

                floorNumber = d.FloorNumber,
                id = d.ClientId,
                FloorName = d.FloorName,
                units = _context.EstateUnits.Where(x => x.FloorId == d.ClientId).Select(x => new UnitVM()
                {
                    floorNumber = d.FloorNumber,
                    isEmptyUnit = x.IsEmpty,
                    id = x.ClientIdForUnit,
            ...
            viewModel.FloorsForShow = floors;
            return viewModel;
        }
        ...
        public async Task<Response<string>> Update(UpsertEstateVM input)
        {
            ...
            var OldFloors = _context.Floors.Where(x => x.EstateId == input.Id).ToList();
            foreach (var f in OldFloors)
            {

                var oldUnit = _context.EstateUnits.Where(x => x.FloorId == f.ClientId);
                _context.EstateUnits.RemoveRange(oldUnit);
        ...
        public async Task<Response<List<EstateVM>>> GetAllForView()
        {

            var find = _context.Estates.Where(d => d.IsActive == true).Select(d => new EstateVM()
            {
                Id = d.Id,
                Title = d.Title,
                Image = d.Image,
            ...
            }).OrderByDescending(x => x.Code).Take(16).AsNoTracking();

            if (find == null)
            {
                return new Response<List<EstateVM>>(404);
            }
            return new Response<List<EstateVM>>(find.ToList());
        }
