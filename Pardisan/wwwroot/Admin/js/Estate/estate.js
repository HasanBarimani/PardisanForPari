...
$(function () {
    ...
    table = $('.ajax-data-table').DataTable({
        ...
        },
        {
            ...
            "render": function (data, type, row, meta) {
                var res = ""
                data == 0 ? res = "تکمیل شده" : res = "جاری";
                return `<p class="text-justify mt-3">` + res + `</p>`;
            },
            "targets": 6
        },
        {
            "render": function (data, type, row, meta) {
                ...
                html += `<a href="/admin/Estate/Upsert?id=` + data + `"  class="btn text-white btn-info btn-sm"><i class="fa fa-edit "></i></a>`;
                html += `<a href="/admin/Estate/Gallery?id=` + data + `"  class="btn text-white btn-warning btn-sm"><i class="fa fa-image "></i></a>`;
                html+="</div>"
                return html;
            },
            "targets": -1
...
function DeleteItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/Estate/DisableEstate?Id=' + id,
...
function RestoreItem(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '/admin/Estate/ActiveEstate?Id=' + id,
                beforeSend: function () { $("button").prop("disabled", true); },
                complete: function () { $("button").prop("disabled", false); },
                success: function (data) {
